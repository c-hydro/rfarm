...
#-----------------------------------------------------------------------------------------
# Script information
script_name='FP ENVIRONMENT - PYTHON3 LIBRARIES FOR RFARM PACKAGE - CONDA'
...
if [ -f $fp_env_file_requirements ] ; then
	...
	conda create --yes --name $fp_env_folder_libraries numpy scipy shapely==1.8.5 pandas rasterio netCDF4 cfgrib=0.9.8.5 cython xarray cartopy matplotlib gdal bottleneck dask pip python=3.9
