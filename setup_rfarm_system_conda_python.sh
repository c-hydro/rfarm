...
#-----------------------------------------------------------------------------------------
# Script information
script_name='RFARM ENVIRONMENT - PYTHON LIBRARIES FOR PACKAGE - RUNNER TYPE - CONDA'
...
fp_env_tag_default='rfarm_runner'
...
if [ -n "${fp_env_file_requirements:-}" ] && [ -f "$fp_env_file_requirements" ]; then
  ...
  conda create -y -n "$fp_env_folder_libraries" -c conda-forge \
    "python=3.10" "pip" \
    "numpy" "scipy" "pandas" "xarray" "matplotlib" "shapely" "bottleneck" "dask" "cython" \
    "rasterio>=1.3,<1.4" "gdal>=3.9,<3.10" "cfgrib" "cartopy" \
