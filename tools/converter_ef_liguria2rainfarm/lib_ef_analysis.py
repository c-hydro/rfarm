...
# -------------------------------------------------------------------------------------
# Find slopes value for Liguria
def find_slopes_liguria(list_EF_liguria,a2dTabSlope):
    ...
    slope_y_data = np.zeros(iNumAA)
    slope_t_data = np.zeros(iNumAA)
    pth_data = np.zeros(iNumAA)

    a1dP12table=np.array(a2dTabSlope[:,0])

    for iaa in range(0,iNumAA):
        dP12=float(a1dRain[iaa])
        dP3=float(a1dRain3H[iaa])

        #check P12
        a1dRU=np.unique(a2dTabSlope[:,0])
        dDiffMin=np.nanmin(np.abs(a1dRU-dP12))
        jmin=np.argmin(np.abs(a1dRU-dP12))

        dP12t=a1dRU[jmin]

        a1dJ=np.argwhere(a1dP12table==dP12t)
        #tabella select
        tabella=a2dTabSlope[a1dJ[:],:]

        #Select P3
        a1dP3tmp=tabella[:,0,1]

        jmin3 = np.argmin(np.abs(a1dP3tmp - dP3))

        slope_y_data[iaa]=tabella[jmin3,0,2]
        slope_t_data[iaa] = tabella[jmin3, 0, 3]
        pth_data[iaa] = tabella[jmin3,0,5]
    ...
    return slope_y_data, slope_t_data,pth_data
